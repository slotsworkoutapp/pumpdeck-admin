...
export default function MusclesList() {
  ...
  const targetCount = useMemo(() => {
    ...
    for (const e of catalog?.exercises ?? []) {
      // A collection's pool is the exercises filed under it, which live in
      // collection_id now rather than posing as a primary muscle.
      const ids = [e.primary_muscle_id, e.collection_id, ...e.additional_primary_muscle_ids].filter(Boolean) as string[];
    ...
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{catalog.muscles.length} muscles &amp; collections</p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {orderedMuscles.map((m) => (
              <tr key={m.id} className="cursor-pointer hover:bg-slate-50" onClick={() => nav(`/muscles/${m.id}`)}>
                ...
                <td className="px-4 py-2 text-slate-500">{m.group_raw === 'focus' ? 'collection' : m.group_raw}</td>
